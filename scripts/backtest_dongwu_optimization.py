...
# --- 配置参数 ---
FUND_CODE = "004253"  # 国泰黄金ETF联接C
FUND_NAME = "国泰黄金ETF联接C"
START_DATE = "2023-01-01"
END_DATE = "2025-12-31"  # 覆盖最近3年
INVEST_AMOUNT = 50000.0  # 每次定投金额
INITIAL_CASH = 100000000.0  # 充足本金，模拟不限

# 费率配置 (C类)
# 申购费: 0
# 赎回费: <7天 1.5%, >=7天 0% (参考常见C类黄金ETF联接)
REDEEM_FEE_7D = 0.015
REDEEM_FEE_OLD = 0.0
...
def run_backtest():
    ...
    # 应该获取更早的数据。
    # 修改 get_fund_data 获取 2022-10-01 开始的数据用于预热
    pre_start_date = "2022-10-01"
    ...
    for i, day_data in enumerate(backtest_data):
        ...
        year_return = get_return(250) # 250交易日
        ...
        for acc in accounts:
            ...
            if is_scheduled:
                ...
                # Actually increase.py says:
                # if period_type == 3 and times <= 1: return True (Pass MA5)
                # And later: if is_first_investment (times==1): return True (Pass Profit Check)
                
                # Let's follow increase.py flow:
                ...
                # Low Rank (<20/100 or <5/30) -> Price is relatively Low -> Stop Buy (Don't catch falling knife?)
                # High Rank (>90/100) -> Price is relatively High -> Stop Buy (Don't chase high?)

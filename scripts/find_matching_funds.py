"""
脚本逻辑说明：
本脚本旨在寻找历史上频繁出现（>=10次）且当前处于"底部改善"状态的基金。

具体逻辑如下：
1. 候选池筛选：
   - 从数据库 `fund_investment_indicators` 表中提取基金。
   - 条件：该基金在历史记录中出现次数 >= 10 次。

2. 排名走势获取：
   - 遍历候选基金，调用 `FundRankDiagram` 接口获取近 3 个月的同类排名数据。
   - 计算归一化排名（百分位 = 排名 / 总数）。

3. 核心筛选算法（同时满足以下两点）：
   A. 处于历史低位（相对位置 > 0.8）：
      - 计算当前排名在过去 3 个月区间内的相对位置。
      - > 0.8 表示当前排名数值处于历史最高的 20% 区间（通常排名数值越大代表业绩越靠后/越差），即处于"历史最差区域"。
   B. 短期趋势向好（斜率 < -0.02）：
      - 计算最近 5 个数据点的变化斜率。
      - < -0.02 表示排名数值正在快速下降（排名提升），显示出业绩改善的迹象。

4. 结果：
   - 输出符合上述"超跌"且"反弹"特征的基金列表，并保存到日志文件。
"""
import os
import sys
import json

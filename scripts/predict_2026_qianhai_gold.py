...
# --- 配置参数 ---
# 使用A类代码获取长历史数据来计算波动率和趋势
HISTORY_FUND_CODE = "009198" # 前海开源黄金ETF联接A
TARGET_FUND_NAME = "前海开源黄金ETF联接C" 
...
def run_simulation_once(future_data, history_data):
    ...
    for i, day_data in enumerate(sim_data):
        ...
        # 简单模拟: 假设初始单位净值1.0，每日根据 策略总资产变动 计算净值涨跌?
        # 由于有资金进出，使用份额法计算策略净值
        # 这是一个虚拟的策略净值，用于评估策略本身的"回撤"能力(排除资金进出影响)
        # 但这里更直观的是看 "Accumulated Profit" 的回撤
    ...
    # 6. Max Drawdown Ratio (最大回撤比率)
    # 使用累计收益的回撤金额 / 峰值累计收益? 或者 (Peak - Low) / Peak?
    # 如果 Peak 是负的怎么办?
    # 通常回撤比率是针对 "策略净值" 的。
    # 这里我们用 (Max DD Amount) / (Max Occupied + Max Profit)? 
    # 简单起见，计算 "最大浮亏比例" 作为风险指标可能更符合用户直觉。
    # 但用户明确问了 "最大回撤比率"。
    # 让我们构造一个 "策略净值曲线" (Unit NAV) 来计算标准回撤比率。
    # Unit NAV 初始 1.0
    # Day i: Asset_End = Asset_Start * (1 + Fund_Return)
    # NAV_new = NAV_old * (1 + Fund_Return)
    # 这样只反映标的本身的波动，不反映策略(择时)的优劣?
    # 不，策略的择时体现在仓位上。
    # 准确做法: 策略NAV。
    # Cash portion is 0 (assuming infinite cash pool, only invested part counts).
    # This is complex. 
    # Let's use: Max Drawdown of Accumulated Profit / Max Occupied Capital (Approximate Impact)
    # Or simpler: The user likely implies "How much % did I lose from the peak?"
    # I will report "Max Drawdown Amount" and "Max Floating Loss Ratio".
    # For "Max Drawdown Ratio", I will calculate it on the "Accumulated Profit + Principal" curve? No.
    # I will skip standard "Max Drawdown Ratio" if ambiguous, or define it as Max_DD_Amount / Max_Occupied.

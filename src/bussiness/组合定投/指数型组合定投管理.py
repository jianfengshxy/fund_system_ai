    ...
    try:
        ...
        for indicator in indicators_data:
            ...
            if fund_type == '000' and fund_sub_type in ('000001','000003'):
                ...
                print(f"  跳过基金: {fund_name}({fund_code}) - 类型: {fund_type or '未知'}, 子类型: {fund_sub_type or '未知'}")
        
        if not index_funds:
            print("❌ 没有找到符合条件的指数基金（fund_sub_type: 000001）")
            return
        
        print(f"筛选出 {len(index_funds)} 个符合条件的指数基金")
        
        # 检查这些基金的跟踪指数是否已有定投计划
        print("检查指数基金跟踪指数是否重复...")
        recommended_funds = []
        
        for indicator in index_funds:
            fund_code = indicator.fund_code
            fund_name = indicator.fund_name
        ...
        if not recommended_funds:
            ...
            print(f"\n💡 建议为以下 {len(recommended_funds)} 个指数基金创建定投计划:")
            
            for i, indicator in enumerate(recommended_funds, 1):
                fund_code = indicator.fund_code
                fund_name = indicator.fund_name
                print(f"  {i}. {fund_name}({fund_code}) - 建议定投: {investment_amount:,.0f} 元")
                
                # 自动创建定投计划
                try:
                    print(f"  🚀 正在为 {fund_name}({fund_code}) 创建定投计划，金额: {investment_amount:,.0f} 元")
...
def dissolve_plan_by_group_for_index_funds(user: User, sub_account_name: str, budget: float):
    ...
    Args:
        ...
        budget: 预算金额
    """
    print(f"🔄 开始解散指数型组合 '{sub_account_name}' 的定投计划，预算: {budget:,.2f} 元")
    ...
    try:
        ...
        if not plans_to_dissolve:
            print("✅ 没有找到需要解散的指数基金定投计划")
        else:
            print(f"\n💡 准备解散以下 {len(plans_to_dissolve)} 个指数基金定投计划:")
            ...
            print(f"\n✅ 指数型组合 '{sub_account_name}' 定投计划解散分析完成")
        
    except Exception as e:
        print(f"❌ 处理指数型组合定投计划解散时发生错误: {str(e)}")
        import traceback
        print(f"错误详情: {traceback.format_exc()}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='指数型组合定投管理')

...
def create_table_if_not_exists():
    """
    Create user_total_account_fund_asset_daily table if it doesn't exist
    """
    db = DatabaseConnection()
    sql = """
    CREATE TABLE IF NOT EXISTS user_total_account_fund_asset_daily (
        ...
        PRIMARY KEY (date, customer_no, fund_code),
        INDEX idx_customer_date (customer_no, date)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户总账户基金每日资产明细表';
    """
    try:
        ...
        cursor.close()
        db.disconnect(conn)
        logger.info("Table user_total_account_fund_asset_daily check/creation completed.")
...
def get_all_fund_codes(user: User) -> Set[str]:
    """
    Get all unique fund codes from Base Account and Sub Accounts
    """
    codes = set()
    ...
    try:
        ...
        if base_assets:
            for asset in base_assets:
                if asset.fund_code:
                    codes.add(asset.fund_code)
        logger.info(f"Found {len(codes)} funds in Base Account")
    except Exception as e:
                ...
                if assets:
                    for asset in assets:
                        if asset.fund_code:
                            codes.add(asset.fund_code)
                            count += 1
            logger.info(f"Found {count} fund records in Sub Accounts")
    except Exception as e:
        logger.error(f"Error fetching sub account assets: {e}")
        
    logger.info(f"Total unique funds found: {len(codes)}")
    return codes

def sync_total_account_fund_asset_daily(user: User):
    """
    Sync user total account fund asset data to database
    ...
    try:
        ...
        # 2. Get all fund codes
        fund_codes = get_all_fund_codes(user)
        if not fund_codes:
            logger.warning("No funds found for user.")
            return

        records = []
        
        # 3. Fetch details for each fund
        for fund_code in fund_codes:
            try:
                # Call the required API
                asset = get_fund_asset_details_of_base_sub_hdt(user, fund_code)
                
                if not asset:
                    logger.warning(f"Failed to get details for fund {fund_code}")
                    continue
                
                # Extract data
                # Note: get_fund_asset_details_of_base_sub_hdt returns AssetDetails object
                # Some fields might be None or need parsing
                ...
                record = {
                    ...
                    "fund_type": str(asset.fund_type) if asset.fund_type else "",
                ...
                logger.error(f"Error processing fund {fund_code}: {e}")
                
        logger.info(f"Processed {len(records)} total account fund records.")

        # 4. Save to DB
        if not records:
            logger.warning("No total account fund asset records to save.")
            return
        ...
        sql = """
        INSERT INTO user_total_account_fund_asset_daily 
        (date, customer_no, fund_code, fund_name, fund_type, asset_value, 
        ...
        cursor.close()
        db.disconnect(conn)
        logger.info(f"Successfully synced {len(records)} total account fund asset records.")

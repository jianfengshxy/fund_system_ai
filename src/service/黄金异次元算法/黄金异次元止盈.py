...
def redeem_gold_dimension_funds(user: User, sub_account_name: str) -> bool:
    """
    黄金异次元止盈逻辑：
    收益率大于1.0% 就买出0费率份额
    """
    logger.info(f"开始执行黄金异次元止盈检查，组合: {sub_account_name}", extra={"account": user.account, "sub_account_name": sub_account_name, "action": "gold_dimension_redeem"})
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 3. 调用止盈函数
        redeem_gold_dimension_funds(test_user, test_sub_account)

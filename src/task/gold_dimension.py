...
from src.bussiness.黄金异次元.increase import increase as increase_biz
from src.bussiness.黄金异次元.redeem import redeem as redeem_biz
...
def increase(event, context):
    try:
        ...
        success = increase_biz(user, sub_account_name, amount)
        if success:
...
def redeem(event, context):
    try:
        ...
        success = redeem_biz(user, sub_account_name)
        if success:

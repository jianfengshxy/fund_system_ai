...
def test_super_transfer_and_revoke():
    """测试基金超级转换并撤销"""
    # 打印测试开始信息
    logger.info("开始测试基金超级转换并撤销")
    ...
    # 调用超级转换函数
    logger.info(f"调用超级转换函数: fund_code={fund_code}, share_id={share.shareId}, amount={transfer_amount}")
    result = super_transfer(DEFAULT_USER, sub_account_no, fund_code, transfer_amount, share.shareId)
    
    # 验证转换结果
    assert result is not None, "转换结果不应为None"
    logger.info(f"超级转换结果: {result}")
    ...
    if result and result.busin_serial_no:
        ...
        logger.warning("转换失败或未返回交易ID，无法撤销")
    
    logger.info("测试完成")

if __name__ == "__main__":
    # 直接运行测试
    logger.info("直接运行超级转换测试")
    test_super_transfer_and_revoke()